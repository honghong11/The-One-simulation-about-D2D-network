...
/**
 * Abstract superclass for all reports. All settings defined in this class can
 * be used for all Report classes. Some reports don't implement intervalled
 * reports ({@link #INTERVAL_SETTING}) and will ignore that setting. Most of the
 * reports implement warm up feature ({@link #WARMUP_S}) but the implementations
...
public abstract class Report {
	...
	/**
	 * The interval (simulated seconds) of creating new settings files -setting id
	 * ({@value})
	 */
	...
	/**
	 * Precision of formatted double values - setting id ({@value}). Defines the
	 * amount of decimals shown in formatted double values. Default value is
	 * {@value #DEF_PRECISION}.
	 */
	...
	/**
	 * The default output directory of reports (can be overridden per report with
	 * {@link Report#OUTPUT_SETTING}) -setting id ({@value})
	 */
	public static final String REPORTDIR_SETTING = "Report.reportDir";
	/**
	 * Warm up period -setting id ({@value}). Defines how many seconds from the
	 * beginning of the simulation should not be included in the reports.
	 * Implementation of the feature is report specific, so check out the respective
	 * report classes for details. Default is 0. Must be a positive integer or 0.
	 */
	...
	/**
	 * Constructor. Looks for a className.output setting in the Settings and if such
	 * is found, uses that as the output file name. Otherwise
	...
	public Report() {
		...
		scenarioName = settings
				.valueFillString(settings.getSetting(SimScenario.SCENARIO_NS + "." + SimScenario.NAME_S));
	...
	/**
	 * Checks that a directory for a file exists or creates the directory if it
	 * didn't exist.
	 * 
	 * @param outFileName
	 *            Name of the file
	...
	/**
	 * Initializes the report output. Method is called in the beginning of every new
	 * report file. Subclasses must call this method first in their own
	...
	private void createOutput(String outFileName) {
		try {
			...
			throw new SimError("Couldn't open file '" + outFileName + "' for report output\n" + e.getMessage(), e);
		}
	}
	 ...
	 * 
	 * @param outFileName
	 *            Prefix of the output file's name
	 */
	...
	/**
	 * This method should be called before every new (complete) event the report
	 * logs. If the report has no meaningful use for multiple reports, the call can
	 * be omitted (then only single output file will be generated)
	 */
	...
	/**
	 * Returns a Settings object initialized for the report class' name space that
	 * uses {@value REPORT_NS} as the secondary name space.
	 * 
	...
	/**
	 * Called when the simulation is done, user requested premature termination or
	 * intervalled report generating decided that it's time for the next report.
	 */
	public void done() {
		if (out != null) {
			out.close();
	...
	/**
	 * Returns the average of double values stored in a List or "NaN" for empty
	 * lists.
	 * 
	 * @param values
	 ...
	 * @param values
	 *            The list of values
	 * @return average of integer values stored in the List or "NaN" for empty
	 *         lists.
	 */
